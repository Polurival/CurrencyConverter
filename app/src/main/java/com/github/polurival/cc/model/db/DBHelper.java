...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_NAME_MY_CURRENCY_NET_NOMINAL = "my_currency_net_nominal";
    ...
    public static final String COLUMN_NAME_MY_CURRENCY_NET_RATE = "my_currency_net_rate";
    ...
    public static final String COLUMN_NAME_MY_CURRENCY_NET_SOURCE = "my_currency_net";
    ...
    private void updateDatabase(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 8) {
            db.beginTransaction();
            try {
                db.execSQL("ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + COLUMN_NAME_MY_CURRENCY_NET_NOMINAL + " INTEGER;");
                db.execSQL("ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + COLUMN_NAME_MY_CURRENCY_NET_RATE + " REAL;");
                db.execSQL("ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + COLUMN_NAME_MY_CURRENCY_NET_SOURCE + " INTEGER;");

                // some of currencies that was in Yahoo doesn't exist in myCurrency.net
                db.execSQL(String.format(Locale.ENGLISH, "UPDATE %s SET %s = %d " +
                                "WHERE %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s'" +
                                " AND %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s'" +
                                " AND %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s' AND %s <> '%s'" +
                                " AND %s <> '%s' AND %s <> '%s' AND %s <> '%s';",
                        TABLE_NAME,
                        COLUMN_NAME_MY_CURRENCY_NET_SOURCE, 1,
                        COLUMN_NAME_CHAR_CODE, CharCode.AWG,
                        COLUMN_NAME_CHAR_CODE, CharCode.BTN,
                        COLUMN_NAME_CHAR_CODE, CharCode.BYN,
                        COLUMN_NAME_CHAR_CODE, CharCode.CLF,
                        COLUMN_NAME_CHAR_CODE, CharCode.CNH,
                        COLUMN_NAME_CHAR_CODE, CharCode.CUC,
                        COLUMN_NAME_CHAR_CODE, CharCode.ERN,
                        COLUMN_NAME_CHAR_CODE, CharCode.MXV,
                        COLUMN_NAME_CHAR_CODE, CharCode.SHP,
                        COLUMN_NAME_CHAR_CODE, CharCode.WST,
                        COLUMN_NAME_CHAR_CODE, CharCode.XAG,
                        COLUMN_NAME_CHAR_CODE, CharCode.XAU,
                        COLUMN_NAME_CHAR_CODE, CharCode.XCP,
                        COLUMN_NAME_CHAR_CODE, CharCode.XDR,
                        COLUMN_NAME_CHAR_CODE, CharCode.XPD,
                        COLUMN_NAME_CHAR_CODE, CharCode.XPT,
                        COLUMN_NAME_CHAR_CODE, CharCode.ZMW,
                        COLUMN_NAME_CHAR_CODE, CharCode.ZWL));

                if (isUpgrade) {
                    AppPreferences.saveRateUpdaterClassName(context,
                            context.getString(R.string.my_currency_net_rate_updater_class));
                }
    ...
    public static Cursor getSearchCursor(String charCode, String rateUpdaterClassName,
        ...
        } else if (AppContext.getContext().getString(R.string.my_currency_net_rate_updater_class)
                .equals(rateUpdaterClassName)) {
            sourceColumnName = COLUMN_NAME_MY_CURRENCY_NET_SOURCE;
        } else {
            custom = true;
        }
